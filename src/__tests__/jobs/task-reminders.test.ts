...
describe("task-reminders jobs", () => {
  ...
  describe("runAllReminderJobs", () => {
    it("runs all jobs and returns a summary", async () => {
      ...
      vi.mocked(db.trackingStage.findMany).mockResolvedValue([])
      vi.mocked(db.invoice.findMany).mockResolvedValue([])
      vi.mocked(db.iMForm.findMany).mockResolvedValue([])
      vi.mocked(db.iMForm.updateMany).mockResolvedValue({ count: 0 } as never)
      vi.mocked(db.invoice.updateMany).mockResolvedValue({ count: 0 } as never)
      vi.mocked(db.jobRun.deleteMany).mockResolvedValue({ count: 0 } as never)
      ...
      expect(result).toHaveProperty("stageSlaAlerts")
      expect(result).toHaveProperty("paymentReminders")
      expect(result).toHaveProperty("invoiceReminders")
      expect(result).toHaveProperty("imExpiryAlerts")
      expect(result).toHaveProperty("purgedJobRuns")
