...
// Sync projects with tasks - creates task entries for each project
export async function syncProjectsWithTasks() {
  try {
    ...
    // Get existing tasks with projectIds
    const existingTasks = await db.task.findMany({
      where: { projectId: { not: null } },
      select: { projectId: true },
    });

    const existingProjectIds = new Set(existingTasks.map(t => t.projectId));

    // Create tasks for projects that don't have one
    const newTasks = projects
      .filter(p => !existingProjectIds.has(p.id))
      .map(p => ({
        task: `Project: ${p.customer}`,
        project: p.customer,
        projectId: p.id,
    ...
    return { success: true, message: `Synced ${newTasks.length} new tasks` };
  } catch (error) {
