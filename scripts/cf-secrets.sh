...
# KEYCHAIN OVERRIDES — Vercel refuses env writes, so every value rotated here lives in the macOS Keychain as cf-mazin-<VAR>. Anything stored there
# wins, so re-running this from a stale dotenv cannot regress a fixed secret.
security dump-keychain 2>/dev/null \
  | grep -o '"svce"<blob>="cf-mazin-[A-Z0-9_]*"' | sed 's/.*="cf-mazin-//; s/"$//' | sort -u \
  | while read -r VAR; do
      V=$(security find-generic-password -a "$USER" -s "cf-mazin-$VAR" -w 2>/dev/null || true)
