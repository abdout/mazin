// mazin (abdoutgroup.com) on Cloudflare Containers — a thin Worker in front of one
// container running the Next standalone server. Mirrors the kun, hogwarts and mkan
// lanes; every request is forwarded as-is so the app sees the real Host header.
import { Container, getContainer } from "@cloudflare/containers"
import crons from "./crons.json"

export class MazinContainer extends Container {
  defaultPort = 3000
  // Sleeps after 2 h idle — the lane bills per instance-hour; a cold start is ~10 s.
  sleepAfter = "2h"
...
export default {
  async fetch(request, env) {
    return getContainer(env.MAZIN, "main").fetch(request)
  },
  ...
  async scheduled(controller, env, ctx) {
    ...
    const run = async (path) => {
      ...
      try {
        const res = await getContainer(env.MAZIN, "main").fetch(
